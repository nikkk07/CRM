...
export default function EmployeeDirectory() {
  ...
  const unmarkLeaveDay = async (date) => { try { const token = localStorage.getItem('token'); await fetch(`${API_URL}/api/employees/${selectedEmployee.id}/leave/${date}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } }); showToast('Leave unmarked', 'success'); loadEmployeeDetail(selectedEmployee.id); } catch { showToast('Failed to unmark leave', 'error'); } };
